...
class PredictorsForm(Form):
    ...
    """
    satisfaction_level = StringField(
        u'Satisfaction Level (Valid Values: 0.00-1.00)', validators=[validators.input_required()])
    last_evaluation = StringField(
        u'Commitment (Valid Values: 0.00-1.00)', validators=[validators.input_required()])
    number_project = StringField(u'Number of Projects(Eg.:0,1,2,...)', validators=[
                                 validators.input_required()])
    avg_monthly_hours = StringField(
        u'Average Monthly Hours (Eg.: 0,1,2,...)', validators=[validators.input_required()])
    time_spent = StringField(u'Time Spent in Organisation (In Years)', validators=[
                             validators.input_required()])
    work_accident = StringField(u'Work Accident (Yes:1,No:0)', validators=[
                                validators.input_required()])
    promotion = StringField(u'Promotion In Last 5 years (Yes:1,No:0)', validators=[
                            validators.input_required()])
    salary = StringField(u'Salary (High:0,Low:1,Medium:2)',
...
def predict():
    ...
    if request.method == 'POST' and form.validate():
        ...
        predict_request = [float(satisfaction_level), float(last_evaluation), int(number_project), int(
            avg_monthly_hours), int(time_spent), int(work_accident), int(promotion), int(salary)]
...
def train():
    ...
    predictors = ['satisfaction_level', 'last_evaluation', 'number_project',
                  'avg_monthly_hours', 'time_spent', 'work_accident', 'promotion', 'salary']
    label = 'left'
    ...
    df_train, df_test, y_train, y_test = train_test_split(
        df[predictors], df[label], test_size=0.20, random_state=42)
